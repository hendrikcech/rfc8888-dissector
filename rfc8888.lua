do
    ...
    rfc_proto.fields = { rfc_version_f, rfc_padding_f, rfc_fmt_f, rfc_pt_f, rfc_length_f, rfc_ssrc_rtcp_f,
                         ...
                         rfc_pkts_block_f, rfc_pkt_block_f, rfc_pkt_rcvd_f, rfc_pkt_arrival_offset_f,
                         rfc_report_ts_f }
    ...
    function decode_ssrc(buffer, pinfo, root, report_ts, c)
        ...
        local pkts_tree = tree:add(rfc_pkts_block_f, buffer(c, tree_end))
        local i_pkt = 0
        local not_rcvd = {}
        c = c + 8
        while (num_reports_actual > i_pkt) do
            ...
            local pkt_tree_text = ""
            if (received ~= 0) then
                pkt_tree_text = string.format("%d received at %f (offset %f ms)", begin_seq:uint()+i_pkt, report_ts+arrival_time, arrival_time*1000)
            else
                pkt_tree_text = string.format("%d not yet received", begin_seq:uint()+i_pkt)
                table.insert(not_rcvd, begin_seq:uint()+i_pkt)
            end
        ...
        local not_rcvd_str = format_not_rcvd(not_rcvd)
        if (#not_rcvd > 0) then
            tree:add_proto_expert_info(rfc_not_received_e, not_rcvd_str)
        end

        -- TODO: not a great place to put this
        local info_str = "ACKs "..begin_seq:uint().."-"..(begin_seq:uint()+num_reports_actual)..". "..not_rcvd_str
        pinfo.cols.info:set(info_str)


        not_rcvd_tbl[pinfo.number][ssrc_rtp:uint()] = not_rcvd
    ...
    not_rcvd_tbl = {} -- { pinfo.number: { ssrc: [not_rcvd] } }
